...
export function ProductCard({ product }: { product: Product }) {
  ...
  return (
    <Card className={cn(
      ...
      {/* Badges Overlay - Extreme corners to avoid covering image */}
      <div className="absolute top-2 left-2 md:top-4 md:left-4 z-20 flex flex-col gap-2 pointer-events-none">
        {hasDiscount && (
          <Badge className="bg-red-500 text-white border-none px-2 py-0.5 md:px-3 md:py-1 rounded-lg md:rounded-xl font-black text-[7px] md:text-[10px] shadow-2xl uppercase tracking-widest animate-pulse">
      ...
      <CardContent className="p-4 md:p-8 flex flex-col flex-grow space-y-3 md:space-y-6 bg-gradient-to-b from-transparent to-slate-50/30">
        <div className="space-y-1.5">
          <div className="flex items-center gap-2">
            ...
            {product.isFeatured && (
              <Badge className="bg-amber-100 text-amber-600 border-none font-black text-[7px] md:text-[8px] px-2 py-0.5 rounded-full flex items-center gap-1">
                <Sparkles className="w-2 h-2 md:w-2.5 md:h-2.5" /> SPECIAL
              </Badge>
            )}
          </div>
          <Link href={`/product/${product.id}`}>
            <h3 className="font-black text-xs md:text-xl text-slate-900 line-clamp-1 leading-tight group-hover:text-primary transition-colors tracking-tighter">
              {product.name}
            </h3>
          </Link>
        </div>
        ...
        <div className="pt-2 md:pt-4 mt-auto flex items-center justify-between border-t border-slate-100/50">
          <div className="flex flex-col">
            <div className="flex items-baseline gap-1.5 md:gap-2">
              <span className="font-black text-sm md:text-2xl text-slate-900 tracking-tighter">
