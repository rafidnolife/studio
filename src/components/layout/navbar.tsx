...
import { Home, ShoppingBag, User, ShieldCheck, Heart, LogIn, Sparkles, LogOut } from 'lucide-react';
    ...
    <nav className={cn(
      ...
      <div className="container mx-auto px-4 h-full flex items-center justify-between">
        <Link href="/" className="flex items-center gap-3 group">
          {user ? (
            <div className="flex items-center gap-3 bg-white/60 glass px-6 py-2.5 rounded-full shadow-lg border-primary/10">
              <div className="w-10 h-10 bg-primary rounded-full flex items-center justify-center text-white shadow-lg">
                <User className="w-5 h-5" />
              </div>
              <div className="flex flex-col">
                <span className="text-[9px] font-black text-primary uppercase tracking-[0.2em] flex items-center gap-1">
                  <Sparkles className="w-2.5 h-2.5 animate-pulse" /> স্বাগতম
                </span>
                <span className="font-black text-sm md:text-base text-slate-900 tracking-tighter truncate max-w-[120px] md:max-w-[200px]">
            ...
            <div className="flex items-center gap-3">
              <div className="w-12 h-12 bg-primary rounded-2xl flex items-center justify-center text-white font-black text-2xl shadow-xl shadow-primary/20 group-hover:scale-110 transition-all duration-500">
                ড
              </div>
        ...
        <div className="flex items-center gap-2 md:gap-4">
          <Button variant="ghost" size="icon" className="rounded-2xl hover:bg-red-50 hover:text-red-500 h-12 w-12" asChild>
            <Link href="/wishlist">
              <Heart className="w-6 h-6" />
            </Link>
          </Button>
          {!loading && !user ? (
            <Button className="rounded-2xl px-8 h-12 font-black text-sm shadow-xl shadow-primary/20 uppercase tracking-widest group" asChild>
              <Link href="/login">
            ...
            <Button variant="outline" size="icon" className="rounded-2xl border-2 h-12 w-12 hover:bg-slate-50 border-slate-100 shadow-sm" asChild>
              <Link href="/profile">
